...
-- CPU 1
function parts.cpu1(colorBg)

 local cpu1 = [[\
${if_match ${cpu cpu1}<50}\
]] .. _h.value('${cpu cpu1}%', ci.good) .. [[
${else}${if_match ${cpu cpu1}<60}\
]] .. _h.value('${cpu cpu1}%',ci.degraded) .. [[
${else}${if_match ${cpu cpu1}<=100}\
]] .. _h.value('${cpu cpu1}%', ci.bad) .. [[
${endif}${endif}${endif}\
]]

  return _h.compose('', 'CPU', cpu1, colorBg)
end
-- Battery
