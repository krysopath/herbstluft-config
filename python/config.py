...
keybinds = {
  ...
  # use your $TERMINAL with xterm as fallback
    m+'-Return'   : 'spawn ${TERMINAL:-gnome-terminal}', 

  # epsi
    m+'-d'        : 'spawn dmenu_run_hlwm',
    m+'-'+s+'-d'     : 'spawn rofi -show run -opacity 90',
    m+'-'+s+'-x'     : 'spawn sh -c "xlock"',

    m+'-q': 'spawn gnome-screenshot',
  ...
  # resizing frames
    ...
    m+'-'+a+'-p'     : 'spawn gnome-terminal -e "ncmpc"',
}
...
rules = {
  ...
  # zero based array
    ...
    'class=Gentoo'   : 'tag=' + str(tag_names[2]),
